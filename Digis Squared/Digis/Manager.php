<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST['approve'])) {
            $hoursID = $_POST['approve'];

            $query = "UPDATE work_hours SET approved = 1 WHERE workID=?";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "i", $hoursID);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
        }
        
        elseif (isset($_POST['unapprove'])) {
            $hoursID = $_POST['unapprove'];

            $query = "DELETE FROM work_hours WHERE workID = ?";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "i", $hoursID);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
    ...
    $query = "SELECT WH.*, E.employeeName, E.employeeID
              ...
              WHERE WH.approved != 1 AND M.managerID = '$managerID'
...
<head>
    ...
    $(document).ready(function() {
        $(".clapprove").click(function(event) {
            event.preventDefault();
            var button = $(this);
            var hoursId = button.closest('.Hours').data('hours-id');

            $.ajax({
                type: "POST",
                url: "Manager.php",
                data: { approve: hoursId },
                success: function(response) {
                    button.closest('.Hours').remove();
                },
                error: function() {
        ...
        $(".clremove").click(function(event) {
            ...
            $.ajax({
                type: "POST",
                url: "Manager.php",
                data: { unapprove: hoursId },
                success: function(response) {
                    button.closest('.Hours').remove();
                },
                error: function() {
                    alert('Error unapproving hours');
                }
            });
        });
    });
...
<body>
    ...
    <?php if (!empty($hoursData)): ?>
        <h2 class="section__header" id="remove">Approve Working Hours</h2>
        <div id="allHours">
            <?php foreach ($hoursData as $hours): ?>
                <div class="Hours" data-hours-id="<?= htmlspecialchars($hours['workID']) ?>">
                    ...
                    <button type="button" class="clapprove">Approve</button>
                    <br>
                    <button type="button" class="clremove">Unapprove</button>
                </div>
            <?php endforeach; ?>
        </div>
    <?php else: ?>
